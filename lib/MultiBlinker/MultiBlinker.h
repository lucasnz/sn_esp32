...
// Define error state constants
                                                // LED State:
const int KNIGHT_RIDER = -1;                    // Knight Rider animation or 3000ms blink
const int STATE_NONE = 0;                       // ON: (nothing)
const int STATE_STARTED_WIFI_AP     = 15;        // ON: ALL or solid ON
const int STATE_WIFI_NOT_CONNECTED  = 1;        // ON: 3 or 250ms blink
const int STATE_MQTT_NOT_CONNECTED  = 4;        // ON: 1 or 1000ms blink
